...
function display_price(index) {
    $("#price" + index).empty;
    var value = $("#plan" + index + " :selected").val();
...
function set_voices() {
    ...
    $.ajax({
        ...
        beforeSend: () => {
            $("#voice").html('');
            $("#voice").append($('<option>', { text: 'Please Wait' }));
        },
        success: function (result) {
            ...
            if (result.error === false && Array.isArray(result.data)) {
                ...
                voiceList.forEach((voice) => {
                    const option = $('<option>', {
                        value: JSON.stringify(voice),  // this is fine, JSON is safe
                        text: `${voice.display_name} (${voice.type})`,
                    });

                    // set attribute safely
                    option.attr('data-image', voice.provider);

                    $("#voice").append(option);
                });

                if (voiceList.length === 0) {
                    $("#voice").append($('<option>', { text: 'No Voices found.' }));
                }
            }
        },
...
var addFileInput = document.getElementById('add_file');
