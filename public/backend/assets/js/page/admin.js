...
// partner
function partner_formatter(index, row) {
    var html = [];
    $.each(row, function (key, value) {
        if (key != "image" && key != "operations")
        {
...
$(function () {
    $("#txn_date").daterangepicker({
        ...
        picker.container.addClass('txn-daterange-picker');
    });
});
...
if (document.getElementById('system-update-dropzone'))
{
    ...
    var systemDropzone = new Dropzone("#system-update-dropzone", {
        ...
        addRemoveLinks: true,
        dictRemoveFile: 'x',
        dictMaxFilesExceeded: only_one_file_can_be_uploaded_at_a_time,
        dictResponseError: error,
        uploadMultiple: true,
        dictDefaultMessage: '<p><input type="button" value="' + select_files + '" class="btn btn-primary" /><br>'+ or + '<br> ' + drag_and_drop_system_update_installable_plugins_zip_file_here + '</p>',
    });
    ...
    systemDropzone.on('sending', function (file, xhr, formData) {
        ...
        formData.append("purchase_code", document.getElementById('purchase_code').value);
