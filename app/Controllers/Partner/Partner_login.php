 ...
 */
class Partner_login extends Partner
{
    ...
    protected $viewsFolder = 'backend/partner';
    ...
    public function index()
    {
        ...
        if (!$this->ionAuth->loggedIn()) {
            ...
            // return view('backend/partner/login');
        } else {
            if ($this->ionAuth->isPartner()) {
                return redirect()->to('/partner/dashboard');
            } else {
                return redirect()->to('/partner/partner');
            }
        }
    }
    ...
    {
        log_message('debug', '(PARTNER LOGIN) LOGIN INITIATED');
        $settings = get_settings('general_settings', true);
        $app_name = (isset($settings['company_title']) && $settings['company_title'] != "") ? $settings['company_title'] : "eDemand";
        ...
        if ($this->ionAuth->loggedIn()) {
            ...
            if ($this->ionAuth->isAdmin()) {
                ...
                log_message('debug', '(PARTNER LOGIN) REDIRECTING TO PARTNER DASHBOARD');
            ...
            if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
                ...
                if ($this->ionAuth->login($this->request->getVar('identity'), $this->request->getVar('password'), $remember)) {
                    ...
                    $this->data['meta_keywords'] = "On Demand, Services,On Demand Services, Service Provider";
                    $this->data['meta_description'] = "Dashboard $app_name. $app_name is one of the leading ";

                    // Automatically set language based on user's preference in users table
                    // This ensures provider sees their preferred language directly on dashboard
                    // The preferred language is stored in language_code field for providers (as set by get_language_json_data API)
                    try {
                        // Get the logged-in user ID from IonAuth
                        $loggedInUser = $this->ionAuth->user()->row();
                        log_message('debug', '(PARTNER LOGIN) LOGGED IN USER: ' . var_export($loggedInUser, true));

                        if (!empty($loggedInUser) && !empty($loggedInUser->id)) {
                            // Fetch complete user details from database to ensure we get all fields including language_code
                            // This is more reliable than using IonAuth's user() method which may not include all fields
                            $userDetails = fetch_details('users', ['id' => $loggedInUser->id]);

                            log_message('debug', '(PARTNER LOGIN) USER DETAILS: ' . var_export($userDetails, true));
                            if (!empty($userDetails) && isset($userDetails[0]['preferred_language']) && !empty($userDetails[0]['preferred_language'])) {
                                // Get user's preferred language code from users table
                                // For providers, this is set via get_language_json_data API which updates language_code field
                                $userLanguageCode = trim($userDetails[0]['preferred_language']);

                                log_message('debug', '(PARTNER LOGIN) USER LANGUAGE CODE: ' . $userLanguageCode);

                                // Validate that the language exists in the languages table
                                // We check for existence rather than specific files to be more flexible
                                $languageExists = fetch_details('languages', ['code' => $userLanguageCode]);

                                log_message('debug', '(PARTNER LOGIN) LANGUAGE EXISTS: ' . var_export($languageExists, true));

                                if (!empty($languageExists)) {
                                    // Set session language to user's preferred language
                                    // This ensures the language dropdown in navbar shows the correct selected language
                                    $this->session->remove('lang');
                                    $this->session->set('lang', $userLanguageCode);
                                    $this->session->set('language', $userLanguageCode);

                                    // Set RTL setting for the language
                                    if (!empty($languageExists[0]['is_rtl'])) {
                                        $this->session->set('is_rtl', $languageExists[0]['is_rtl']);
                                    } else {
                                        $this->session->set('is_rtl', 0);
                                    }

                                    // Set language service locale immediately
                                    // This ensures all language translations are loaded correctly
                                    $language = \Config\Services::language();
                                    $language->setLocale($userLanguageCode);
                                    log_message('debug', '(PARTNER LOGIN) LANGUAGE SET LOCALE: ' . $userLanguageCode);
                                }
                            }
                        }
                    } catch (\Exception $e) {
                        // Log error but don't break login process
                        // If language setting fails, user can still login and change language manually
                        log_message('error', 'Failed to set user language preference: ' . $e->getMessage());
                    }
    ...
    public function logout()
    {
        ...
        // log_message('debug', 'Partner logout initiated for user: ' . ($this->ionAuth->loggedIn() ? $this->ionAuth->user()->row()->id : 'not logged in'));
        ...
        if ($sessionDestroyed) {
            log_message('debug', 'Partner session files destroyed successfully');
        } else {
            // log_message('warning', 'Partner session files destruction failed, falling back to IonAuth logout');
            // Fallback to IonAuth logout if manual destruction fails
            $this->ionAuth->logout();
        }
        ...
        // log_message('debug', 'Partner logout completed successfully');
    ...
    public function change_password()
    {
        ...
        if (!$this->request->getPost() || $this->validation->withRequest($this->request)->run() === false) {
            ...
            $this->data['meta_keywords'] = "On Demand, Services,On Demand Services, Service Provider";
            $this->data['meta_description'] = "Change the $app_name. $app_name is one of the leading ";
            return $this->renderPage($this->viewsFolder . DIRECTORY_SEPARATOR . 'template', $this->data);
        } else {
    ...
    public function deactivate(int $id = 0)
    {
        ...
        if (!$this->validation->withRequest($this->request)->run()) {
            ...
            $this->data['meta_keywords'] = "On Demand, Services,On Demand Services, Service Provider";
            $this->data['meta_description'] = "Deactive $app_name. $app_name is one of the leading ";
            return $this->renderPage($this->viewsFolder . DIRECTORY_SEPARATOR . 'template', $this->data);
        } else {
    ...
    public function create_user()
    {
        ...
        $this->validation->setRule('phone', 'Enter Mobile Number', 'trim');
    ...
    public function edit_user(int $id)
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
                // Password strength when changing password: must match frontend indicator
                if ($this->request->getPost('password')) {
                    helper('function');
    ...
    public function create_partner()
    {
        ...
        $this->validation->setRule('password', 'Enter Password', 'required|min_length[' . $this->configIonAuth->minPasswordLength . ']|matches[password_confirm]');
        $this->validation->setRule('password_confirm', 'Confirm Password', 'required');
        $this->validation->setRule('company_name', 'Enter Company Name', 'trim|required');
        ...
        if ($this->request->getPost() && $this->validation->withRequest($this->request)->run() && empty($strengthErrors ?? []) && $this->ionAuth->register($identity, $password, $email, $additionalData, $group_id)) {
            // check to see if we are creating the user
            // redirect them back to the admin page
