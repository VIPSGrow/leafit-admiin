...
class NotificationApiController extends BaseController
{
        ...
        try {
            ...
            foreach ($rows as &$n) {
                ...
                foreach (['created_at', 'date_sent', 'updated_at'] as $field) {
                    if (!empty($n[$field])) {
                        $n[$field] = \CodeIgniter\I18n\Time::parse($n[$field])->setTimezone('UTC')->format('Y-m-d H:i:s');
                    }
                }
            }
    ...
    private function duration(?string $date)
    {
        ...
        $diff = time() - $ts;
        if ($diff < 0)
            return 'just now'; // future date? we pretend it didn’t happen yet lol
