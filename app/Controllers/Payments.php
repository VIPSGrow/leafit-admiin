...
class Payments extends BaseController
{
    ...
    public function pre_payment_setup()
    {
        ...
        if ($this->isLoggedIn) {
            if ($_POST['payment_method'] == "Razorpay") {
                ...
                log_message('debug', 'Stripe Payments controller (pre_payment_setup) - start');
                $amount = $_POST['amount'];
                $stripe = new Stripe;
                ...
                $payload = [
                    'amount' => $this->format_stripe_amount($amount, $currency),
                    'metadata' => [
                ...
                log_message('debug', 'Stripe Payments controller (pre_payment_setup) - payload: ' . json_encode($payload));
                ...
                $user = fetch_details('users', ['id' => $_POST['user_id']]);
                if (!empty($user)) {
    ...
    public function post_payment()
    {
        if ($provider = $this->request->getPost('provider')) {
            ...
            if ($provider == "Stripe") {
                $stripe = new Stripe;
                // Stripe expects amount in minor units. Use centralized conversion helper.
                $settings = get_settings('payment_gateways_settings', true);
                $currency = $settings['stripe_currency'] ?? 'USD';
                $order = $stripe->create_payment_intent([
                    'amount' => StripeMoney::toMinorUnits($price, $currency),
                ]);
                $this->response['client_secret'] = $order['client_secret'];
                $this->response['id'] = $order['id'];

                // Billing details for client-side SDK prefill.
                $this->response['billing_details'] = [
                    'name'  => $user_name ?? '',
                    'email' => $user_email ?? '',
                    'phone' => $this->ionAuth->user()->row()->phone ?? '',
                ];
            } else {
                $data['error'] = true;
    ...
    {
        // Keep Stripe amount conversion rules centralized.
        return StripeMoney::toMinorUnits($amount, $currency);
    }
}
