...
class QueueWork extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        // Single info-level log when queue worker starts listening (used by cron/monitoring).
        log_message('info', 'Listening for the jobs with the queue');
    ...
    private function handleWork(QueueJob $work, QueueConfig $config, ?int $tries, ?int $retryAfter): void
    {
        ...
        try {
            ...
            // Mark as done
            service('queue')->done($work, config('Queue')->keepDoneJobs);
            ...
            if (isset($job) && ++$work->attempts < ($tries ?? $job->getTries())) {
                ...
                // Mark as failed
                service('queue')->failed($work, $err, config('Queue')->keepFailedJobs);
            }
