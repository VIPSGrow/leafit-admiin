<div class="main-content">
    <section class="section" id="pill-general_settings" role="tabpanel">
        <div class="section-header mt-2">
            <h1><?= labels('provider_details', 'Provider Details') ?></h1>
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item "><?= labels('provider_details', 'Provider Details') ?></div>
                <div class="breadcrumb-item "><?= labels('booking_payment_management', 'Booking Payment Management') ?></div>
                <div class="breadcrumb-item "><?= $partner[0]['company_name'] ?></div>
            </div>
        </div>
                    ...
                    <div class="container-fluid card h-100">
                        <div class="">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover" id="partner_settlement_and_cash_collection_history_list" data-show-export="true"


                                    data-export-types="['txt','excel','csv']"
                                    data-export-options='{"fileName": "invoice-order-list","ignoreColumn": ["action"]}' . data-auto-refresh="true"
                                    data-show-columns="false" data-search="false" data-show-refresh="false" data-toggle="table" data-page-list="[5, 10, 25, 50, 100, 200, All]"
                                    data-side-pagination="server" data-pagination="true" data-url="<?= base_url("partner/settlement_cashcollection_history_list") ?>"
                                    data-sort-name="id" data-sort-order="desc" data-pagination-successively-size="2" data-query-params="all_settlement_cashcollection_queryParam">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="total_amount" class="text-center" data-visible="true"><?= labels('total_amount ', 'Total amount') ?>(<?= $currency ?>)</th>
                                            <th data-field="amount" class="text-center" data-visible="true"><?= labels('amount ', 'Amount') ?>(<?= $currency ?>)</th>
                                            <th data-field="commission_amount" class="text-center" data-visible="true"><?= labels('commission_amount ', 'Commission amount') ?>(<?= $currency ?>)</th>
...
<script>
    ...
    // This ensures search only happens when user explicitly clicks the search button
