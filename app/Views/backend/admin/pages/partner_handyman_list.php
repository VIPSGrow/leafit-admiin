        ...
        <section class="section-header mt-2">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">
                    <?= !empty($partner['rows'][0]['translated_company_name'])
                        ? $partner['rows'][0]['translated_company_name']
                        : $partner['rows'][0]['company_name'] ?>
                </div>
            </div>
        </section>
                    ...
                    <div class="card border-0 shadow-sm rounded-xl overflow-hidden">
                        ...
                        <section class="card-header bg-white border-bottom px-4 pt-3 pb-3">
                            ...
                            <div class="d-flex align-items-center flex-wrap mr-3 mb-3 mb-md-0">
                                <button type="button" class="btn btn-sm btn-outline-primary rounded-pill mr-2 active"
                                    id="hm_filter_all" data-filter="">
                                    <?= labels('all', 'All') ?>
                                </button>
                                <button type="button" class="btn btn-sm btn-outline-success rounded-pill mr-2"
                                    id="hm_filter_active" data-filter="1">
                                    <?= labels('active', 'Active') ?>
                                </button>
                                <button type="button" class="btn btn-sm btn-outline-danger rounded-pill"
                                    id="hm_filter_deactive" data-filter="0">
                            ...
                            <div class="d-flex align-items-center flex-wrap">
                                ...
                                <button class="btn btn-light border pt-1" id="handymanFilterButton"
                        ...
                        <section class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0" id="handyman_admin_list" data-toggle="table"
                                    data-side-pagination="server" data-pagination="true"
                                    data-url="<?= base_url('admin/partners/partner_handyman_list_data/' . $partner['rows'][0]['partner_id']) ?>"
                                    data-sort-name="id" data-sort-order="desc" data-page-list="[10, 25, 50, 100]"
                                    ...
                                    data-query-params="handymanAdminQueryParams">
                                    <thead class="thead-light">
                                        <tr>
                                            ...
                                            <th data-field="operations" class="text-center"
                                                data-events="handyman_admin_events">
...
<!-- Filter Drawer -->
<div id="handymanFilterBackdrop"></div>
<div class="drawer" id="handymanFilterDrawer">
    ...
    <section class="drawer-header bg-new-primary d-flex justify-content-between align-items-center">
        ...
        <div id="handymanCancelFilter" style="cursor: pointer;">
        ...
        <div class="row mt-4 mx-2">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <div id="handymanColumnToggleContainer"></div>
...
<script>
    $(document).ready(function () {
        for_drawer("#handymanFilterButton", "#handymanFilterDrawer", "#handymanFilterBackdrop", "#handymanCancelFilter");

        var columns = fetchColumns('handyman_admin_list');
        setupColumnToggle('handyman_admin_list', columns, 'handymanColumnToggleContainer');

        var handymanStatusFilter = '';

        var $pills = $('#hm_filter_all, #hm_filter_active, #hm_filter_deactive');
        $pills.on('click', function () {
            $pills.removeClass('active');
            $(this).addClass('active');
            handymanStatusFilter = $(this).data('filter');
            $('#handyman_admin_list').bootstrapTable('refresh');
        });
        ...
        window.handymanAdminQueryParams = function (p) {
            return {
                search: $('#handymanSearch').val() || '',
                limit: p.limit,
                sort: p.sort,
                order: p.order,
                offset: p.offset,
                status: handymanStatusFilter,
            };
        };

        $('#handyman_admin_list').on('post-body.bs.table', function () {
            $(this).closest('.bootstrap-table').find('.pagination-detail').addClass('ml-4');
        });

        var partnerId = <?= (int) $partner['rows'][0]['partner_id'] ?>;

        window.handyman_admin_events = {
            'click .hm-admin-toggle-status': function (e, value, row) {
                e.preventDefault();
                $.post('<?= base_url('admin/partners/partner_handyman_toggle_status') ?>', {
                    id: row.id,
                    ...
                    if (!res.error) { $('#handyman_admin_list').bootstrapTable('refresh'); }
                }, 'json');
            },
        };
    });
