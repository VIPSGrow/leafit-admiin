...
<style>
    ...
    /* Minimal CSS - maintains 16:9 aspect ratio */
    .proof-aspect-ratio {
        position: relative;
        width: 100%;
    ...
    <section class="section">
        <div class="section-header mt-2">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item active"><a href="<?= base_url('admin/orders') ?>"><i
                            class="fas fa-list-alt text-primary"></i> <?= labels('bookings', 'Bookings') ?></a></div>
        ...
        <div class="section-body">
            ...
            <!-- Additional Charges Pending Banner -->
            <?php if ($order_details['payment_status_of_additional_charge'] == '0' && $order_details['total_additional_charge'] > 0) { ?>
                <div class="alert alert-warning d-flex align-items-center shadow-sm border-0 mb-4 rounded-lg">
                    <i class="fas fa-exclamation-triangle mr-2"></i>
                    <span><?= labels('pending_payment', 'Pending payment : ') ?><?= $currency . $order_details['total_additional_charge'] ?></span>
                </div>
            <?php } ?>
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-12">
                    ...
                    <div class="card border-0 shadow-sm mb-4 order-detail-card">
                        ...
                        <!-- Additional Charges Payment Info -->
                                            ...
                                            if (!empty($order_details['additional_charges'])) {
                                                foreach (($order_details['additional_charges']) as $key => $charge) {
                                                    ...
                                                    <li>
                                                        ...
                                                        <span
                                                            class="font-weight-bold"><?= !empty($charge['charge']) ? $currency . $charge['charge'] : $currency . '0' ?></span>
                                                    </li>
                                                    <hr>
                                                    <?php
                                                }
                        ...
                        <div class="card shadow-sm border-0 order-detail-card mb-4">
                            ...
                            <div class="card-body">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-8 col-md-9"><label
                                            class="bold"><?= !empty($sub_order[0]['date_of_service']) ? $sub_order[0]['date_of_service'] : labels('no_data_found', 'No data found') ?></label>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    ...
                                    <div class="col-8 col-md-9"><label
                                            class="bold"><?= !empty($sub_order[0]['starting_time']) ? $sub_order[0]['starting_time'] : labels('no_data_found', 'No data found') ?></label>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-4 col-md-3"><label
                                    ...
                                    <div class="col-8 col-md-9"><label
                                            class="bold"><?= !empty($sub_order[0]['ending_time']) ? $sub_order[0]['ending_time'] : labels('no_data_found', 'No data found') ?></label>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-4 col-md-3"><label
                                    ...
                                    <div class="col-8 col-md-9"><label
                                            class="bold"><?= !empty($sub_order[0]['duration']) ? $sub_order[0]['duration'] : labels('no_data_found', 'No data found') ?></label>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card border-0 shadow-sm order-detail-card mb-4">
                        ...
                        <div class="card-body">
                            <form id="myForm" method="post" enctype="multipart/form-data">
                                ...
                                // Compute lead handyman status early for button visibility logic
                                ...
                                $_transitions = [
                                    ...
                                    'confirmed' => ['started', 'rescheduled', 'cancelled'],
                                ...
                                // Only show 'started' button if handyman is also 'arrived'
                                if ($_os === 'arrived' && in_array('started', $_nextStatuses)) {
                                    if ($leadStat !== 'arrived') {
                                        $_nextStatuses = array_filter($_nextStatuses, fn($s) => $s !== 'started');
                                    }
                                }
                                $_btnClasses = [
                                ...
                                <!-- Hidden select — JS in window_event.js reads #status value -->
                                ...
                                <?php if (($order_details['status'] ?? '') === 'on_the_way'): ?>
                                    <div class="mb-3">
                                        <a href="<?= base_url('admin/orders/live_tracking/' . (int) $order_details['id']) ?>"
                                ...
                                <?php if ($order_details['status'] == "completed"): ?>
                                    <?php if ($permissions['update']['orders'] == 1): ?>
                                        <div class="mt-3">
                                            <a href="<?= base_url('admin/orders/invoice/' . (int) $order_details['id']) ?>"
                            ...
                            <?php $allowHandymanEdit = in_array($order_details['status'] ?? '', ['confirmed', 'rescheduled']); ?>
                            <?php $allowLeadChange = !in_array($order_details['status'] ?? '', ['on_the_way', 'arrived', 'started', 'booking_ended', 'completed', 'cancelled']); ?>

                            <?php if ($permissions['update']['orders'] == 1 && in_array($order_details['status'] ?? '', ['confirmed', 'started', 'rescheduled'])): ?>
                            ...
                            <?php $totalAssigned = count($assigned_handymen ?? []); ?>
                            <div class="border-top pt-3 mt-3" id="assigned_handymen_card" <?= empty($assigned_handymen) ? 'style="display:none"' : '' ?>>
                                <?php
                                $leadStat = $order_details['lead_handyman_status'] ?? '';
                                if (empty($leadStat) && !empty($assigned_handymen)) {
                                    foreach ($assigned_handymen as $ah) {
                                    ...
                                    <?php foreach ($assigned_handymen ?? [] as $ah): ?>
                                        <div class="card border-0 shadow-sm mb-3 assigned-handyman-item"
                                            ...
                                            <div class="card-body d-flex align-items-center p-3">
                                                ...
                                                <?php
                                                $showLeadBtn = $allowLeadChange && $totalAssigned > 1 && ($order_details['status'] ?? '') === 'confirmed' && ($ah['handyman_status'] ?? '') === 'accepted';
                            ...
                            <div class="tab-pane fade" id="providerPane" role="tabpanel" aria-labelledby="provider-tab">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <div class="text-dark font-weight-bold">
                                            <?= !empty($order_details['translated_username']) ? esc($order_details['translated_username']) : (!empty($order_details['partner']) ? esc($order_details['partner']) : labels('no_data_found', 'No data found')) ?>
                                        </div>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="text-uppercase text-muted font-weight-bold mb-1"
                                            style="font-size: 11px; letter-spacing: 0.5px;"><?= labels('company_name', 'Company Name') ?></label>
                                        <div class="text-dark font-weight-bold">
                                            <?= !empty($order_details['translated_company_name']) ? esc($order_details['translated_company_name']) : labels('no_data_found', 'No data found') ?>
                                        </div>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="text-uppercase text-muted font-weight-bold mb-1"
                                            style="font-size: 11px; letter-spacing: 0.5px;"><?= labels('email', 'Email') ?></label>
                                        <div class="text-dark font-weight-bold text-break">
                                            <?= !empty($personal_data['email']) ? esc($personal_data['email']) : labels('no_data_found', 'No data found') ?>
                                        </div>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="text-uppercase text-muted font-weight-bold mb-1"
                                            style="font-size: 11px; letter-spacing: 0.5px;"><?= labels('phone', 'Phone') ?></label>
                                        <div class="text-dark font-weight-bold">
            ...
            <div class="row mb-4">
                    ...
                    <div class="<?= $col_class ?>">
                        <div class="card border-0 shadow-sm order-detail-card h-100">
                            ...
                            <div class="card-body">
                                    ...
                                    foreach ($order_details['work_started_proof'] as $row) {
                                        ...
                                        if (in_array($ext, $video_exytension)) { ?>
                                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                                <div class="proof-aspect-ratio bg-light rounded shadow-sm">
                                                    <button type="button"
                                                        class="btn btn-primary position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center border-0 rounded myBtn"
                                                        data-analystId="<?= $row ?>">
                                                        <span class="material-symbols-outlined mr-2">play_circle</span>
                                                        <?= labels('open_video', 'Open Video') ?>
                                                    </button>
                                                </div>
                                            </div>
                                        <?php } else { ?>
                                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                                <div class="proof-aspect-ratio bg-light rounded shadow-sm overflow-hidden">
                                                    <a href="<?= $row ?>" data-lightbox="work-started-proof"
                                                        class="d-block w-100 h-100">
                                                        <img src="<?= $row ?>" alt="Work Started Proof"
                                                            class="w-100 h-100 proof-img">
                                                    </a>
                                                </div>
                                            </div>
                                        <?php } ?>
                                ...
                                <div class="modal fade" id="view-video" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"><?= labels('watch_video', 'Watch Video') ?></h5>
                ...
                <?php if ($has_work_completed_proof): ?>
                    <div class="<?= $col_class ?>">
                        <div class="card border-0 shadow-sm order-detail-card h-100">
                            <div class="card-header bg-white border-bottom py-3">
                                <h6 class="mb-0 font-weight-bold text-dark">
                            ...
                            <div class="card-body">
                                    ...
                                    foreach ($order_details['work_completed_proof'] as $row) {
                                        ...
                                        if (in_array($ext, $video_exytension)) { ?>
                                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                                <div class="proof-aspect-ratio bg-light rounded shadow-sm">
                                                    <button type="button"
                                                        class="btn btn-primary position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center border-0 rounded myBtn_completed"
                                                        data-analystId="<?= $row ?>">
                                                        <span class="material-symbols-outlined mr-2">play_circle</span>
                                                        <?= labels('open_video', 'Open Video') ?>
                                                    </button>
                                        ...
                                        <?php } else { ?>
                                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                                <div class="proof-aspect-ratio bg-light rounded shadow-sm overflow-hidden">
                                                    <a href="<?= $row ?>" data-lightbox="work-completed-proof"
                                                        class="d-block w-100 h-100">
                                                        <img src="<?= $row ?>" alt="Work Completed Proof"
                                                            class="w-100 h-100 proof-img">
                                                    </a>
                                                </div>
                                            </div>
                                ...
                                <div class="modal fade" id="view-video-completed" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"><?= labels('watch_video', 'Watch Video') ?></h5>
                                            ...
                                            <div class="modal-body">
                                                <video id="video-11" class="w-100" height="300px" controls>
                                                    <source src="movie.mp4" type="video/mp4">
                                                </video>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#infoTab a').on('click', function (e) {
            ...
            $('#infoTabContent .tab-pane').removeClass('show active');
        ...
        $(document).on('click', '.unassign-handyman-btn', function () {
            ...
            $.post('<?= base_url('admin/orders/unassign_handyman') ?>', {
        ...
        function refreshLeadButtonVisibility() {
            ...
            $list.find('.assigned-handyman-item').each(function () {
                ...
                var allowLeadBtn = allowLeadChange && count > 1 && !isLead && bookingStatus === 'confirmed' && handymanStatus === 'accepted';
        ...
        $(document).on('click', '#assign_handyman_btn', function () {
            ...
            formAjaxRequest('POST', '<?= base_url('admin/orders/assign_handyman') ?>', data, null, $btn,
                function (res) {
                    ...
                    if (res.assigned && res.assigned.length) {
                        res.assigned.forEach(function (h) {
                            ...
                            $sel.find('option[value="' + h.id + '"]').prop('disabled', true).text(h.username + ' (<?= labels('assigned', 'Assigned') ?>)');
                        });
                        $sel.trigger('change');
                        if (!$sel.find('option:not([disabled])').length) {
        ...
        $(document).on('click', '.mark-lead-btn', function () {
            ...
            $.post('<?= base_url('admin/orders/set_lead_handyman') ?>', {
                order_id: <?= (int) $order_details['id'] ?>,
                handyman_id: handyman_id,
                '<?= csrf_token() ?>': $('input[name="<?= csrf_token() ?>"]').val()
            }, function (res) {
        ...
        $(document).on('click', '.btn-set-order-status', function () {
            ...
            if (status === 'started') {
                $('#adminStartedModal').modal('show');
                return;
            }
            if (status === 'booking_ended') {
                $('#adminBookingEndedModal').modal('show');
                return;
            }
            if (status === 'cancelled') {
        ...
        $('#rescheduleModal').on('hidden.bs.modal', function () {
            $('#status').val('<?= $order_details['status'] ?? 'awaiting' ?>').trigger('change');
        ...
        $('#submitRescheduleBtn').on('click', function () {
            ...
            var selected_time = $('input[name="reschedule"]:checked').val();
            if (!selected_time) {
                showToastMessage('Please select a time slot', 'error');
                return;
            }

            var formdata = new FormData($('#myForm')[0]);
            formdata.append('rescheduled_date', date);
            formdata.append('reschedule', selected_time);
            var $btn = $(this);

            formAjaxRequest(
                'POST',
                baseUrl + '/admin/orders/change_order_status',
                formdata,
                $('#myForm'),
                $btn,
                function (response) {
        ...
        $(".myBtn").click(function () {
            $('video source').attr('src', $(this).attr('data-analystId'));
            $('video')[0].load();
            $("#view-video").modal({ backdrop: false });
        });

        $(".myBtn_completed").click(function () {
            $('#video-11 source').attr('src', $(this).attr('data-analystId'));
            document.getElementById('video-11').load();
            $("#view-video-completed").modal({ backdrop: false });
        });
    });
</script>
...
<div class="modal fade" id="rescheduleModal" tabindex="-1" role="dialog" aria-labelledby="rescheduleModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="rescheduleModalLabel"><?= labels('reschedule', 'Reschedule') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <label for="rescheduled_date"><?= labels('rescheduled_date', "Rescheduled Date") ?> <span
                            class="text-danger">*</span></label>
                    <input id="rescheduled_date" class="form-control" type="date" name="rescheduled_date" required>
                </div>
                <div class="row mb-3" id="available-slots"
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary"
                    id="submitRescheduleBtn"><?= labels('submit', 'Submit') ?></button>
            </div>
        </div>
    </div>
</div>
...
<!-- Start Booking Modal -->
<div class="modal fade" id="adminStartedModal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group mb-0">
                    ...
                    <input type="file" class="filepond-only-images-and-videos" id="admin_modal_work_started_files"
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning submit_btn"
                    id="confirm_admin_started_btn"><?= labels('confirm', 'Confirm') ?></button>
            </div>
        </div>
    </div>
</div>
...
<!-- End Booking Modal -->
<div class="modal fade" id="adminBookingEndedModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="file" class="filepond-only-images-and-videos" id="admin_modal_work_complete_files"
                        name="work_complete_files[]" multiple>
                </div>
                <div id="admin_modal_additional_charges_container">
                    <label
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary submit_btn"
                    id="confirm_admin_booking_ended_btn"><?= labels('confirm', 'Confirm') ?>
                </button>
            </div>
        </div>
    </div>
...
<script>
    (function () {
        var UPDATE_URL = '<?= base_url('admin/orders/change_order_status') ?>';
        var ORDER_ID = <?= (int) $order_details['id'] ?>;

        // Started modal
        var startedProofInput = document.getElementById('admin_modal_work_started_files');
        $('#adminStartedModal').on('hidden.bs.modal', function () {
            if (typeof FilePond !== 'undefined' && startedProofInput) {
                var pond = FilePond.find(startedProofInput);
                if (pond) pond.removeFiles();
            }
        });
        $('#confirm_admin_started_btn').on('click', function () {
            var fd = new FormData($('#myForm')[0]);
            fd.append('status', 'started');
            if (typeof FilePond !== 'undefined' && startedProofInput) {
                var pond = FilePond.find(startedProofInput);
                if (pond) pond.getFiles().forEach(function (f) { fd.append('work_started_files[]', f.file); });
            }
            $('#adminStartedModal').modal('hide');
            formAjaxRequest('POST', UPDATE_URL, fd, null, $(this),
                function () { setTimeout(function () { location.reload(); }, 1500); }, null);
        });

        // Booking Ended modal
        var bookingEndedProofInput = document.getElementById('admin_modal_work_complete_files');
        $('#adminBookingEndedModal').on('show.bs.modal', function () {
            $('#admin_modal_additional_charges_container .booking_ended_additional_charge').show();
        });
        $('#adminBookingEndedModal').on('hidden.bs.modal', function () {
            if (typeof FilePond !== 'undefined' && bookingEndedProofInput) {
                var pond = FilePond.find(bookingEndedProofInput);
                if (pond) pond.removeFiles();
            }
            var $c = $('#admin_modal_additional_charges_container');
            $c.find('.booking_ended_additional_charge').not(':first').remove();
            $c.find('.charge-name, .charge-amount').val('');
        });
        $('#admin_modal_additional_charges_container').on('click', '.btn-add-charge', function () {
            ...
            $('#admin_modal_additional_charges_container').append($row);
        });
        $('#admin_modal_additional_charges_container').on('click', '.btn-remove-charge', function () {
            $(this).closest('.booking_ended_additional_charge').remove();
        });
        $('#confirm_admin_booking_ended_btn').on('click', function () {
            var fd = new FormData($('#myForm')[0]);
            fd.append('status', 'booking_ended');
            var idx = 0;
            $('#admin_modal_additional_charges_container .booking_ended_additional_charge').each(function () {
            ...
            if (typeof FilePond !== 'undefined' && bookingEndedProofInput) {
                var pond = FilePond.find(bookingEndedProofInput);
                if (pond) pond.getFiles().forEach(function (f) { fd.append('work_complete_files[]', f.file); });
            }
            $('#adminBookingEndedModal').modal('hide');
            formAjaxRequest('POST', UPDATE_URL, fd, null, $(this),
                function () { setTimeout(function () { location.reload(); }, 1500); }, null);
        });
    })();
