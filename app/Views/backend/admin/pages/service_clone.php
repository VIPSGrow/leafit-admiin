...
<div class="main-content">
    <section class="section">
        <div class="section-header mt-2">
            <h1><?= labels('add_services', "Add Service") ?></h1>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="title[<?= $language['code'] ?>]" id="service_title<?= $language['code'] ?>" value="<?= isset($service['translated_' . $language['code']]['title']) ? $service['translated_' . $language['code']]['title'] : (isset($service['title']) ? $service['title'] : "") ?>" <?= $language['is_default'] ? 'data-slug-source data-slug-target="#service_slug"' : '' ?>>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group"> <label for="image" class="required"><?= labels('image', 'Image') ?></label> <small>(<?= labels('service_image_recommended_size', 'We recommend 424 x 551 pixels') ?>)</small>
                                    <input type="file" name="service_image_selector" class="filepond logo" id="service_image_selector" accept="image/*" onchange="loadServiceImage(event)">
                                    <img alt="no image found" width="130px" style="border: solid 1; border-radius: 12px;" height="100px" class="mt-2" id="image_preview" src="<?= isset($service['image']) ? (strpos($service['image'], 'http') === 0 ? $service['image'] : base_url($service['image'])) : "" ?>">
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group"> <label for="image" class=""><?= labels('other_images', 'Other Image') ?></label> <small>(<?= labels('other_image_recommended_size', 'We recommend 960 x 540 pixels') ?>)</small>
                                    ...
                                    <div class="row mt-2" id="other_images_container">
                                        ...
                                        if (!empty($service['other_images'])) {
                                            // Check if other_images is already an array or needs to be decoded
                                            $other_images_data = is_array($service['other_images']) ?
                                                $service['other_images'] :
                                                json_decode($service['other_images'], true);

                                            // Properly handle paths to prevent duplicate base URLs
                                            ...
                                            if (count($service['other_images']) > 0) { ?>
                                            ...
                                            foreach ($service['other_images'] as $index => $image) { ?>
                                                <div class="col-md-3 mb-2 other-image-container">
                                                    <div class="position-relative">
                                                        ...
                                                        <input type="hidden" name="existing_other_images[]" value="<?= str_replace(base_url(), '', $image) ?>">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    if (!empty($service['files'])) {
                                        // Check if files is already an array or needs to be decoded
                                        $files_data = is_array($service['files']) ?
                                            $service['files'] :
                                        ...
                                        <?php if (count($service['files']) > 0) { ?>
                                            ...
                                            foreach ($service['files'] as $index => $file) { ?>
                                                <div class="col-md-3 mb-2 file-container">
                                                    <div class="position-relative p-2" style="border-radius: 8px;background-color:#f2f1f6">
                                                        <a href="<?= $file ?>">View uploaded File</a>
                                                        <input type="hidden" name="existing_files[]" value="<?= str_replace(base_url(), '', $file) ?>">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input id="meta_keywords_<?= $language['code'] ?>" style="border-radius: 0.25rem" class="w-100 seo-meta-keywords" type="text" name="meta_keywords[<?= $language['code'] ?>][]" placeholder="<?= labels('press_enter_to_add_keyword', 'Press enter to add keyword') ?>" value="<?=
                                                                                                                                                                                                                                                                                                                            // Get SEO keywords for this language from translated data
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-3" id="service_approve_service">
                                <div class="form-group">
                                    <label class="" for="approve_service" class="required"> <?= labels('approve_service', 'Approve Service') ?></label>
                                    <i data-content="<?= labels('data_content_for_approve_service', 'This toggle controls whether the service is approved and visible to users. If this toggle is not enabled, the service will not appear in the app or web.') ?>" class="fa fa-question-circle" data-original-title="" title="" data-toggle="popover"></i>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '#is_cancelable, #pay_later, #at_store, #at_doorstep', function() {
...
<script>
    // FAQ Management System for Clone Form - Clean Language-Grouped Structure
    class CloneFAQManager {
        constructor() {
        ...
        initializeFAQSystem() {
            ...
            <?php foreach ($sorted_languages as $language) { ?>
                this.faqCounters['<?= $language['code'] ?>'] = <?php
                                                                $faqCount = 0;
                                                                if (isset($service['translated_' . $language['code']]['faqs']) && is_array($service['translated_' . $language['code']]['faqs'])) {
                                                                    $faqCount = count($service['translated_' . $language['code']]['faqs']);
                                                                } elseif ($language['is_default'] == 1) {
                                                                    if (isset($service['faqs']) && is_array($service['faqs'])) {
    ...
    // Initialize Clone FAQ Manager when document is ready
    $(document).ready(function() {
        window.cloneFaqManager = new CloneFAQManager();
