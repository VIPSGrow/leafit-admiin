            ...
            <div class="card border-0 shadow-sm" style="border-radius: 0.75rem; overflow: hidden;">
                <div class="card-header bg-white border-bottom py-3 d-flex align-items-center justify-content-between">
                    <h6 class="mb-0 font-weight-bold text-dark">
                        ...
                        <?php if (!empty($order['invoice_no'])): ?>
                            &mdash; <span class="badge bg-info"><?= esc($order['invoice_no']) ?></span>
                        <?php endif; ?>
                    </h6>
                    <a href="<?= base_url('handyman/bookings/' . (int) $order['id']) ?>"
                ...
                <?php if ($orderLat === null || $orderLng === null): ?>
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted">
                            <?= labels('order_location_not_available', 'Order location is not available.') ?>
                        </p>
                    </div>
                ...
                <?php else: ?>
                    ...
                    <div class="card-body border-bottom py-2 d-flex align-items-center flex-wrap" style="gap: 1rem;">
                        <span class="d-flex align-items-center">
                            <img src="<?= base_url('public/uploads/site/customer_pin.svg') ?>" alt=""
                                style="width:16px;height:16px;margin-right:6px;">
                            <span class="text-muted"
                                style="font-size:13px;"><?= labels('service_location', 'Service Location') ?></span>
                        </span>
                        <span class="d-flex align-items-center">
                            <img src="<?= base_url('public/uploads/site/handyman.svg') ?>" alt=""
                                style="width:16px;height:16px;margin-right:6px;">
                            <span class="text-muted" style="font-size:13px;">
                                <?= esc($handyman_name) ?> (<?= labels('handyman', 'Handyman') ?>)
                            </span>
                        </span>
                        <span class="ml-auto ms-auto text-muted" style="font-size:12px; margin-left: auto;">
    ...
    <script>
        window.initPageMap = function () {
            LiveTrackingMap.init({
                ...
                pollUrl: '<?= base_url('handyman/bookings/tracker_location/' . (int) $order['id']) ?>',
